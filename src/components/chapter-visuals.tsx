...
const Character = ({
  ...
  mood?: "happy" | "neutral" | "sad" | "confident";
  color?: string;
}) => {
  const mouth = {
...
export const FresherVisuals = {
  graduation: (
    <Panel bg="bg-gradient-to-br from-[#1686F5]/20 to-[#0B1C9C]/40">
      ...
      <div className="absolute inset-0 flex items-end justify-center pb-10 gap-4">
        {[0, 1, 2].map((i) => (
          ...
          >
            ...
            {i !== 1 && (
              <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-bold bg-[#09B83E] text-white px-2 py-0.5 rounded">
                OFFER!
              </div>
            )}
          </motion.div>
      ...
      <div className="absolute top-6 left-6 text-white/60 text-xs font-semibold tracking-widest uppercase">
        🎓 Graduation Day
      </div>
    </Panel>
  ),
  ...
  discoverSap: (
    <Panel bg="bg-gradient-to-br from-[#1686F5] to-[#0B1C9C]">
      ...
      >
        <div className="text-[120px] font-display font-black text-white drop-shadow-[0_0_40px_rgba(25,199,216,0.8)]">
          SAP
        </div>
      </motion.div>
  ...
  joinNextgen: (
    <Panel bg="bg-gradient-to-br from-[#1686F5]/30 to-[#19C7D8]/20">
      ...
      <div className="absolute top-6 left-6 text-white text-sm font-display font-bold">
        🚪 Welcome aboard
      </div>
    </Panel>
  ),
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="space-y-3 w-3/4">
          {["MM Module", "SD Module", "FICO Module", "ABAP", "S/4 HANA"].map((m, i) => (
            ...
            >
              <div className="h-8 w-8 rounded-full bg-growth flex items-center justify-center text-xs font-bold text-white">
                ✓
              </div>
    ...
    <Panel bg="bg-gradient-to-br from-[#1686F5]/30 to-[#09B83E]/20">
      ...
      <div className="absolute inset-0 p-8 flex flex-col justify-center gap-4">
        ...
        {["Technical Round", "HR Round", "Final"].map((r, i) => (
          ...
          >
            <div className="h-8 w-8 rounded-full bg-growth flex items-center justify-center text-white font-bold">
              ✓
            </div>
    ...
    <Panel bg="bg-gradient-to-br from-[#09B83E]/30 to-[#19C7D8]/20">
      ...
      >
        <div className="bg-white text-[#071126] rounded-lg p-6 w-full shadow-2xl">
          <div className="text-[10px] font-bold tracking-widest text-[#1686F5] mb-2">
            OFFER LETTER
          </div>
      ...
      {Array.from({ length: 15 }).map((_, i) => (
        <motion.div
          ...
          whileInView={{ y: 400, opacity: [0, 1, 0] }}
          transition={{ duration: 2, delay: i * 0.1, repeat: Infinity }}
          className="absolute w-2 h-2 rounded-sm"
          style={{
            left: `${(i * 37) % 100}%`,
            background: ["#1686F5", "#19C7D8", "#09B83E"][i % 3],
          }}
        />
      ))}
    </Panel>
          ...
          >
            <div className="grid grid-cols-2 gap-0.5 p-1">
              {Array.from({ length: Math.floor(h / 15) }).map((_, j) => (
                <div key={j} className="aspect-square bg-[#19C7D8]/40 rounded-sm" />
              ))}
            </div>
          </motion.div>
        ))}
    ...
    <Panel bg="bg-gradient-to-br from-[#1686F5]/20 to-[#071126]">
      ...
      <div className="absolute inset-0 flex items-end justify-center pb-10 gap-3 px-4">
        {[0, 1, 2, 3].map((i) => (
          <motion.div
            ...
            transition={{ delay: i * 0.15 }}
            className="relative"
            style={{ width: 64, height: 110, opacity: i === 3 ? 0.45 : 1 }}
          >
            <Character
              mood={i === 3 ? "sad" : "happy"}
              color={i === 3 ? "#1686F5" : ["#09B83E", "#19C7D8", "#1686F5"][i]}
            />
            {i !== 3 && (
              <div className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] font-bold bg-[#09B83E] text-white px-2 py-0.5 rounded whitespace-nowrap">
                HIRED
              </div>
            )}
          </motion.div>
      ...
      <div className="absolute top-6 left-6 right-6 flex flex-col gap-1.5">
        {["Amit · TCS", "Neha · Infosys", "Rohan · Wipro"].map((t, i) => (
          ...
          >
            <span>{t}</span>
            <span className="text-[#09B83E] font-bold">JOINED</span>
          </motion.div>
        ))}
      </div>
    </Panel>
    ...
    <Panel bg="bg-gradient-to-br from-[#19C7D8]/20 to-[#1686F5]/20">
      ...
      <div className="absolute inset-0 flex items-center justify-center gap-3 p-6">
        <div className="w-20 h-32">
          <Character mood="sad" color="#1686F5" />
        </div>
        <div className="w-20 h-32">
          <Character mood="happy" color="#E94B8C" />
        </div>
      </div>
      <motion.div
      ...
      <div className="absolute bottom-4 left-6 text-[10px] uppercase tracking-widest text-white/50">
        👫 A sister's advice
      </div>
    </Panel>
  ),
    ...
    <Panel bg="bg-gradient-to-br from-[#0B1C9C]/40 to-[#1686F5]/20">
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-center">
        ...
        >
          ...
          <span className="text-[#071126] text-sm font-semibold">
            what is SAP and how to learn it
          </span>
        </motion.div>
        {[
    ...
    <Panel bg="bg-gradient-to-br from-[#1686F5]/20 to-[#09B83E]/20">
      ...
      <div className="absolute inset-0 p-6 flex items-center justify-center gap-6">
        <div className="w-16 h-24">
          <Character mood="confident" color="#1686F5" />
        </div>
        <div className="flex flex-col gap-2 flex-1 max-w-[180px]">
          {["Tell me about SAP MM", "Order to Cash flow?", "Real project challenge?"].map(
            (q, i) => (
              <motion.div
                key={q}
...
export const ExperiencedVisuals = {
  currentJob: (
    <Panel bg="bg-gradient-to-br from-slate-800 to-[#071126]">
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-center gap-3">
        {["📧 240 unread", "📞 12 missed calls", "📊 5 reports due", "⏰ Working till 11 PM"].map(
          (t, i) => (
            <motion.div
              key={t}
              initial={{ x: -30, opacity: 0 }}
        ...
        <div className="mt-3 text-center">
          ...
          <div className="font-display font-black text-2xl text-white/70 line-through">
            ₹3.5 LPA
          </div>
        </div>
      </div>
  ...
  familyRec: (
    <Panel bg="bg-gradient-to-br from-[#19C7D8]/20 to-[#1686F5]/20">
      ...
      <div className="absolute inset-0 flex items-center justify-center gap-4 p-6">
        <div className="w-20 h-32">
          <Character mood="happy" color="#09B83E" />
        </div>
        <div className="comic-bubble text-sm max-w-[180px]">
          You should explore SAP. It changed my career!
        </div>
      </div>
    </Panel>
  ),
    ...
    <Panel bg="bg-gradient-to-br from-[#0B1C9C]/40 to-[#1686F5]/20">
      ...
      <svg viewBox="0 0 300 200" className="absolute inset-0 w-full h-full p-6">
        <motion.path
          ...
          fill="none"
        />
        <text x="150" y="195" fill="white" fillOpacity="0.6" fontSize="10" textAnchor="middle">
          SAP Job Demand 2020 → 2026
        </text>
      </svg>
    ...
    <Panel bg="bg-gradient-to-br from-[#1686F5]/30 to-[#19C7D8]/20">
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-center gap-3">
        ...
        {["Assignment 1", "Assignment 2", "Live Project", "Certification"].map((t, i) => (
          ...
          >
            <div className="h-6 w-6 rounded-full bg-growth flex items-center justify-center text-xs">
              ✓
            </div>
  ...
  newJob: (
    <Panel bg="bg-gradient-to-br from-[#09B83E]/30 to-[#19C7D8]/20">
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-center">
        {[
          ["₹3.5L", "Before", "bg-white/20"],
          ["₹8L", "Year 1", "bg-[#1686F5]"],
          ["₹12L", "Year 2", "bg-[#19C7D8]"],
          ["₹18L", "Year 3", "bg-[#09B83E]"],
        ].map(([v, l, c], i) => (
          <motion.div
            key={l}
            initial={{ width: 0 }}
  ...
  placementProgram: (
    <Panel bg="bg-gradient-to-br from-[#09B83E]/30 to-[#1686F5]/20">
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-center gap-3">
        <div className="text-center">
          <div className="inline-block bg-[#09B83E] text-white text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full">
    ...
    <Panel bg="bg-gradient-to-br from-[#19C7D8]/20 to-[#09B83E]/20">
      ...
      <div className="absolute inset-0 p-6 flex gap-3 items-center">
        ...
        >
          ...
          {[60, 40, 70].map((w, i) => (
            <div key={i} className="h-1.5 bg-white/30 rounded mb-1.5" style={{ width: `${w}%` }} />
          ))}
        </motion.div>
        <motion.div
          initial={{ x: 30, opacity: 0 }}
        ...
        >
          <div className="text-[9px] uppercase text-[#1686F5] font-bold mb-2">
            After · SAP Consultant
          </div>
          {[90, 85, 95, 80, 90].map((w, i) => (
            <div key={i} className="h-1.5 bg-[#1686F5] rounded mb-1.5" style={{ width: `${w}%` }} />
          ))}
        </motion.div>
      </div>
      <div className="absolute bottom-4 left-0 right-0 text-center text-[10px] uppercase tracking-widest text-white/60">
...
export const CareerGapVisuals = {
  ...
  findNextgen: (
    <Panel bg="bg-gradient-to-br from-[#19C7D8]/20 to-[#1686F5]/20">
      ...
      <div className="absolute inset-0 flex items-center justify-center gap-3 p-6">
        <div className="w-20 h-32">
          <Character mood="happy" color="#1686F5" />
        </div>
      ...
      <div className="absolute bottom-6 left-6 right-6 bg-white/10 rounded-lg p-3 text-xs">
        ...
        <div className="flex gap-2">
          {["Learn", "Practice", "Place"].map((s) => (
            <div key={s} className="flex-1 h-1.5 bg-growth rounded-full" />
          ))}
        </div>
      </div>
    </Panel>
  ...
  newBeginning: (
    <Panel bg="bg-gradient-to-br from-[#1686F5]/20 to-[#0B1C9C]/30">
      ...
      <div className="absolute top-6 left-6 right-6 text-center text-white/70 text-xs font-display font-semibold tracking-widest uppercase">
        Confident · Professional · Ready
      </div>
    </Panel>
  ),
  success: (
  ...
  marriage: (
    <Panel bg="bg-gradient-to-br from-[#E94B8C]/25 to-[#1686F5]/25">
      ...
      <div className="absolute top-6 left-6 text-[10px] uppercase tracking-widest text-white/60">
        A New Chapter
      </div>
    </Panel>
  ),
    ...
    <Panel bg="bg-gradient-to-br from-[#19C7D8]/20 to-[#E94B8C]/20">
      ...
      <div className="absolute inset-0 flex items-end justify-center pb-12 gap-2">
        ...
        <motion.div
          ...
          className="w-12 h-20"
        >
          <Character mood="happy" color="#19C7D8" />
        </motion.div>
      </div>
      <div className="absolute top-6 left-6 right-6 grid grid-cols-3 gap-2">
        {["🍼 Care", "🏠 Home", "👶 Love"].map((t) => (
          <div key={t} className="bg-white/10 rounded px-2 py-1 text-[10px] text-center">
            {t}
          </div>
        ))}
      </div>
    ...
    <Panel bg="bg-gradient-to-br from-slate-700/40 to-[#071126]">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-24 h-36">
          <Character mood="sad" color="#E94B8C" />
        </div>
      </div>
      {["Am I too late?", "Can I still learn?", "Will anyone hire me?"].map((q, i) => (
        <motion.div
          ...
          style={{ top: `${15 + i * 25}%`, left: i % 2 ? "60%" : "8%", maxWidth: 140 }}
        >
          {q}
        </motion.div>
      ))}
  ...
  decideRestart: (
    <Panel bg="bg-gradient-to-br from-[#1686F5]/30 to-[#09B83E]/20">
      ...
      <div className="absolute bottom-8 left-6 right-6 text-center">
        <div className="comic-bubble inline-block text-sm">
          I'm restarting. For me. For my family.
        </div>
      </div>
    </Panel>
  ),
  ...
  mockPlace: (
    <Panel bg="bg-gradient-to-br from-[#19C7D8]/20 to-[#09B83E]/25">
      <SoftGrid />
      <div className="absolute inset-0 p-6 flex flex-col justify-center gap-2">
        <div className="text-center text-[10px] uppercase tracking-widest text-[#09B83E] font-bold mb-2">
          Placement Program
        </div>
        {[
          "Mock Interview 1 · ✓",
