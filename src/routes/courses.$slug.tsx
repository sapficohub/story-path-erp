...
function CoursePage() {
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer:
        "Yes. Every student receives live SAP server access for practical training.",
    },
    {
      question: "Is placement assistance available?",
      answer:
        "Yes. We provide placement assistance, resume preparation and interview support.",
    },
    {
      question: "Can I attend a free demo?",
      answer:
        "Yes. You can book a free demo session before enrolling.",
    },
  ];
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 md:grid-cols-[1.4fr_1fr]">
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-2 text-sm">
                <span className="rounded-full bg-white/15 px-3 py-1 backdrop-blur"><Clock className="mr-1 inline h-4 w-4" />{course.duration}</span>
                <span className="rounded-full bg-white/15 px-3 py-1 backdrop-blur"><Award className="mr-1 inline h-4 w-4" />{course.level}</span>
                <span className="rounded-full bg-white/15 px-3 py-1 backdrop-blur"><TrendingUp className="mr-1 inline h-4 w-4" />100% Placement Support</span>
              </div>
              <div className="mt-7 flex flex-wrap gap-3">
                <Link to="/contact" className="rounded-full bg-white px-6 py-3 font-semibold text-brand-dark shadow-glow transition hover:scale-105">Book Free Demo</Link>
                <a href="https://wa.me/919000333859" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-6 py-3 font-semibold text-white"><MessageCircle className="h-4 w-4" /> WhatsApp</a>
              </div>
            </div>
          ...
          <div className="space-y-12 lg:col-span-2">
            ...
            <div>
              ...
              <ul className="mt-4 grid gap-3 sm:grid-cols-2">
                {course.who.map((item) => (
                  <li key={item} className="flex items-start gap-2 rounded-xl border border-border bg-card p-4">
                    <Users className="mt-0.5 h-5 w-5 text-brand" /><span>{item}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <div className="mt-4 grid gap-2">
                {course.curriculum.map((module, index) => (
                  ...
                  >
                    <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-brand text-sm font-bold text-white">{index + 1}</span>
            ...
            <div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {course.careers.map((career) => (
                  <div key={career} className="flex items-center gap-2 rounded-xl bg-secondary p-4 font-semibold">
          ...
          <aside className="self-start space-y-5 lg:sticky lg:top-24">
            <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                {["Live SAP Server Access", "Real-Time Industry Projects", "Certification Guidance", "Resume + Mock Interviews", "Lifetime Recording Access"].map((benefit) => (
                  <li key={benefit} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-brand-green" />{benefit}</li>
                ))}
              </ul>
            </div>
        ...
        <section className="bg-secondary py-16">
          <div className="mx-auto max-w-7xl px-4">
            ...
            <div className="mt-8 grid gap-5 sm:grid-cols-3">
              {related.map((item) => (
                <Link key={item.slug} to="/courses/$slug" params={{ slug: item.slug }} className="rounded-2xl border border-border bg-card p-5 shadow-card transition hover:scale-[1.02]">
